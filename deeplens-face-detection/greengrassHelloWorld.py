...
bucket_name = 'deeplens-facerecognition-t1'
...
def push_to_s3(img):
    try:
        ...
        key = 'image/frame-' + time.strftime("%Y%m%d-%H%M%S") + '.jpg'
        s3 = boto3.client('s3')
        ...
        error_msg = "pushing to s3 failed: " + str(e)
        client.publish(topic=iot_topic, payload=error_msg)
        ...
        while True:
            ...
            for obj in parsed_inference_results[model_type]:
                if obj['prob'] > detection_threshold:
                    if (output_map[obj['label']] == 'face'):
                        detectedface = True
                        break
                    
            if(detectedface):
                frame_upload = cv2.resize(frame, (672, 380))
                push_to_s3(frame_upload)
                play_audio()
                time.sleep(1)
